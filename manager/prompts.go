...
func ParsePromptFile(promptFilePath string) (map[string]string, error) {
	...
	// Read file content
	fileContent, err := utils.ReadFile(promptFilePath)
	if err != nil {
		return nil, err
	}
	...
	if len(parsedFile.Prompts) > 0 {
		for _, prompt := range parsedFile.Prompts {
			...
			if prompt.Validate == config.VALIDATION_NONE {
				placeHolderValue, err := utils.AskData(prompt.Prompt)
				if err != nil {
					return nil, err
				}
			...
			if strings.HasPrefix(prompt.Validate, config.VALIDATION_NUMBER) {
				placeHolderValue, err := utils.AskDataWithValidation(prompt.Prompt, validators.Number)
				if err != nil {
					return nil, err
				}
			...
			if strings.HasPrefix(prompt.Validate, config.VALIDATION_NUMBER_RANGE) {
				...
				placeHolderValue, err := utils.AskDataWithValidation(prompt.Prompt, validators.NumberRange(validatorOptions))
				if err != nil {
					return nil, err
				}
			...
			if strings.HasPrefix(prompt.Validate, config.VALIDATION_ALPHABET) {
				...
				placeHolderValue, err := utils.AskDataWithValidation(prompt.Prompt, validators.Alphabet(caseSensitiveType, stringLenghtOpts))
				if err != nil {
					return nil, err
				}
